...
class AronsonSet:
    ...
    # Is this actually necessary? Perhaps can be fully generated by forward_generate
    def subset(self, seq: AronsonSequence = None):
        ...
        for i, j in combinations(range(len(seq)), 2):
            if j > i + 1:
                # how to delete two indices in the middle?
                mid_inds = list(range(i + 1, j))
                subsets = power_set(mid_inds)
                for sub in subsets:
                    # Idea- look at minimum range
                    if not sub:
                        # empty subset
                        continue
    ...
    def forward_fix(self, seq: AronsonSequence) -> set[AronsonSequence]:
        ...
        # TODO not good enough- use same range as candidate. Can't generate [18, 28] backwards case
